...
class EventNetClient(val config: EventNetClientConfig, serviceName: String, channelLimit: Int, restartDelayMs: Long) {
     ...
     */
    def sendErrorEvent(Context: String,
        ShortExceptionInfo: String,
        ServiceNameInError: String,
        ServiceIdInError: String,
        ExceptionInfo: String): Unit = {
        ...
        val errorEvent = new ErrorEvent(
            ...
            ServiceNameInError,
            ServiceIdInError,
            ExceptionInfo, agentIdentification)
...
object EventNetClient {
    ...
    def main(args: Array[String]) = {
        ...
        println(prop.toString.replace(',', '\n'))

        if (false) {
            val env = System.getenv
            val keyList = env.keySet.toArray.toList.map(k => k.asInstanceOf[String])
            keyList.map(k => println("env " + k + " : " + System.getenv.get(k)))
            System.exit(99)
        }
